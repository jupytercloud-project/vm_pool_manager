<script lang="ts">
  ...
  const navLinks = $derived(() => {
    ...
    if (isStaff) {
      ...
      // Chercheur : self-service calcul — environnements, jobs batch, consommation.
      links.push({ href: '/environments', label: $_('nav.myEnvironments') });
      links.push({ href: '/jobs', label: $_('nav.jobs') });
      links.push({ href: '/usage', label: $_('nav.costs') });
    } else if (auth) {
