...
<Dropdown simple isOpen={false} class="mt-2">
    {#each configlist as config}
        <DropdownItem name={config.name} onclick={handleClickDropdown}>
            {config.name}
        </DropdownItem>
    {/each}
</Dropdown>
...
{#if textspacedisplay}
    ...
    <Textarea id="textarea-id" placeholder="#!/bin/bash"
         rows={25} bind:value={text} class="w-full"/>
    ...
    <Input id="config-name" type="text" placeholder="Configuration Name"
        class="mt-2 mb-2" bind:value={newconfigname} />
    {#if config_name !== newconfigname}
        <Button size="md" class="w-48 h-12 mt-2" onclick={handlecreateConfig}>
            Save Configuration
        </Button>
        <Button size="md" class="w-48 h-12 mt-2" onclick={handledeleteConfig}
            disabled >
            Delete Configuration
        </Button>
    {:else}
        <Button size="md" class="w-48 h-12 mt-2" onclick={handleupdateConfig}>
